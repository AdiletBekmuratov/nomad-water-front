...
export const DeleteProfile: FC<IDeleteProfileProps> = ({ setVisible, visible, id, name }) => {
  const [deleteProduct, { isLoading: isLoadingDelete }] = useDeleteProfileMutation();
  const handleDelete = async () => {
    toast
      .promise(deleteProduct(id!).unwrap(), {
        loading: 'Loading',
        success: 'Удалено',
  ...
  return (
    <Modal isOpenModal={visible} setIsOpenModal={setVisible}>
      ...
      <h2 className={`text-2xl py-12 text-center font-normal text-red-600`}>
        Удалить {`${name}`}?
      </h2>
